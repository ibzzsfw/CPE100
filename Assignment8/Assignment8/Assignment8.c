...
void print_Data (double data[],int *count)//ใช้แสดงผลชุดข้อมูล
{
    ...
    for (i=0; i<*count; i++)//วนรอบค่า i ตั้งแต่ 0 ถึง *count-1
        printf("Data[%d] = %g\n",i,data[i]);//แสดงผล Data[i]=|Data[i]| และขึ้นบันทัดใหม่
}
int get_menu(double data[],int *count)//เลือกfunctionที่จะทดสอบ
{
    int select;//ใช้เป็นจำนวนเต็มที่นำไปใช้
    printf("************************\n");//แสดงผล ************************ และขึ้นบันทัดใหม่
    printf("*Please select Function*\n");//แสดงผล *Please select Function* และขึ้นบันทัดใหม่
    printf("************************\n");//แสดงผล ************************ และขึ้นบันทัดใหม่
    printf("*    1.Add data        *\n");//แสดงผล *    1.Add data        * และขึ้นบันทัดใหม่
    printf("*    2.Delete data     *\n");//แสดงผล *    2.Delete data     * และขึ้นบันทัดใหม่
    printf("*    3.Statistic       *\n");//แสดงผล *    3.Statistic       * และขึ้นบันทัดใหม่
    ...
    {
        printf("\n***************************\n");//แสดงผล *************************** และขึ้นบันทัดใหม่
        printf("*There is no current data.*");//แสดงผล *There is no current data.* และขึ้นบันทัดใหม่
        printf("\n***************************\n\n");//แสดงผล *************************** และขึ้นบันทัดใหม่
    }
...
void add_Data(double data[], int *count)//ใช้เพิ่มข้อมูล
{
    ...
    do//ทำ
    {
        ...
        if (a<=0)//ถ้า a<=0
            printf("\nSome input denied.\n");//แสดงผล Some input denied. และขึ้นบันทัดใหม่ (ข้อมูลถูกปฏิเสธ)
...
void delete_Data(double data[],int *count)//ใช้ลบข้อมูล
{
    ...
    printf("Input data that you want to delete : ");//แสดงผล Input data that you want to delete :  เพื่อถามว่าอยากลบค่าใด
    ...
    for (i=0; i<*count; i++)//วนรอบค่า i ตั้งแต่ 0 ถึง *count-1
        if (data[i]==n)//ถ้า n มีค่าตรงกับข้อมูลตัวที่ i
        {
            ...
            printf("There's %lf at Data[%d]\n",n,i);//โปรแกรมแจ้งว่ามีค่าข้อมูลนั้นๆที่ตำแหน่งใดเพื่อจะได้ลบให้ถูกตำแหน่ง
            printf("Delete press 1, Don't press 0 : ");//โปรแกรมแจ้งให้ผู้ใช้ตัดสินใจว่าจะลบตำแหน่งนี้จริงหรือไม่ Delete press 1, Don't press 0
            bin = get_int_interval(0, 1);//รับค่าการตัดสินใจของผู้ใช้
            if (bin == 1)//ถ้าผู้ใช้ตัดสินใจลบ
            {
                for (int j=i; j<*count; j++)//วนรอบค่า j ตั้งแต่ i ถึง *count-1 (แก้ไขตำแหน่งตัวถัดๆไป)
                    data[j] = data[j+1];//แก้ไขตำแหน่งตัวถัดๆไปให้ลดลงมา 1
                *count = *count - 1;//ลดจำนวนข้อมูลทั้งหมด 1 ในรอบที่ลบ
            }
        }
}
void statistic(double data[],int count,double *min,double *max,double *mean,double *sd)//ใช้คำนวณค่าทางสถิติ
{
    int i;//ใช้ i วนรอบ
    double sumMean = 0,sumSD = 0;//sumMean คือผลรวมที่จะนำไปหา*mean(ผลรวมกำลังหนึ่ง),sumSD คือผลรวมที่จะนำไปหา*sd(ผลรวมกำลังสอง)
...
int main()
{
    ...
    do//ทำ
    {
        ...
        if ((count==0&&select==2)||(count==0&&select==3))//ถ้าไม่มีข้อมูลแต่เลือกฟังก์ชัน 2,3
            printf("\nThere is no data now, you can not use this function.\n\n");//แสดงผล There is no data now, you can not use this function. และขึ้นบันทัดใหม่
        else if (count!=0)//มิฉะนั้นถ้า count!=0
        {
            if (select==2)//ถ้า select=2
            {
                ...
                if (count==0)//ถ้า count=0
                    printf("\nThere is no data now.\n\n");//แสดงผล There is no data now. และขึ้นบันทัดใหม่
                else//มิฉะนั้น
                    print_Data(data, &count);//เรียกใช้ print_Data
            }
            else if (select==3)//มิฉะนั้นถ้า select=3
            {
                if (count!=1)//ถ้า count!=1
                {
                    ...
                    printf("\nmin = %g\nmax = %g\nmean = %g\nsd = %g\n\n",min,max,mean,sd);//แสดงผลค่าทางสถิติที่ได้จาก statistic
                }
                else//มิฉะนั้น
                    printf("\nFunction available When there are at least 2 data.\n\n");//แสดงผล Function available When there are at least 2 data. และขึ้นบันทัดใหม่
            }
        }
    }

...
/*
เปลี่ยนสตริงเป็นพิมพ์เล็กโดยเปลี่ยนทีละตัว macOS มีปัญหาในการใช้ strlwr โดยตรง หากใช้ใน windows
ไม่จำเป็นต้องมีส่วนตรงนี้
 */
{
...
void add_space(char *buff) //เพิ่มช่องว่างระหว่าง Token
{
  ...
  int i, j;
  unsigned long max;
  for (i = 0, j = 0, max = strlen(buff); i <= max;
       i++) //วนรอบตั้งแต่ 0 ถึงความยาวของสตริง
...
int split(char *buff,
          char token[][20]) //แบ่ง string ใหญ่ๆออกเป็น Token จำนวน count Tokens
{
...
int check_operator(char *str) //ตรวจสอบว่าtoken นั้นเป็นตัวดำเนินการหรือไม่
{
  if ((strlen(str) == 1) &&
      (strchr("+-*/^()", str[0]) !=
       NULL)) //ถ้ามีสตริงที่ยาว 1 อักษรและสตริงนั้นเป็นหนึ่งใน + - * / ^ ( )
    return 1; //รีเทิร์น 1
  else        //มิฉะนั้น
    return 0; //รีเทิร์น 0
}
...
{
  char function_name[15][10] = {
      "sin",  "cos", "tan", "asin", "acos", "atan",
      "sqrt", "pow", "log", "exp",  "abs"}; //รายการฟังก์ชันที่ตรวจสอบได้
  ...
  while (
      i < function_count &&
      strcmp(function_name[i], buff) !=
          0) //ขณะที่ตัวนับเริ่มต้นยังน้อยกว่าตัวนับทั้งหมด และ buff ยังไม่เหมือนกับรายการฟังก์ชันนั้นๆ
  ...
  if ((isalpha(str[0])) || str[0] == '_') //ถ้าตัวแรกเป็นตัวอักษรหรือ _ (ขีดล่าง)
  {
    for (i = 1; i < strlen(str); i++) //วนรอบตั้งแต่ 1ถึง ความยาวสตริง-1
      if (!(isalpha(str[i]) || isdigit(str[i]) ||
            str[i] == '_')) //ถ้าไม่เป็นอักขระ หรือ เป็นตัวเลข หรือ เป็นขีดล่าง
        j = 0;
...
int check_terminate(char *str) //ตรวจสอบว่าtoken นั้นเป็นรูปแบบของคำจบหรือไม่
{
  ...
  while (
      i < terminate_count &&
      strcmp(terminate_name[i], buff) !=
          0) //ขณะที่ตัวนับเริ่มต้นยังน้อยกว่าตัวนับทั้งหมด และ buff ยังไม่เหมือนกับรายการคำจบนั้นๆ
    i++;     //เพิ่มตัวนับไปเรื่อยๆ
